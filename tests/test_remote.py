...
class TestQueryGET(BiothingsWebTest):
    ...
    # Hist Search 1 year
    def test_15(self):
        res = self.query(q='__all__', extra_filter='(date:["2020-01-01" TO "2020-12-31"])', size='0', hist='date', hits=False)
        assert len(res['facets']['hist_dates']['terms']) == 1

    # hist search multiple years
    def test_16(self):
        res = self.query(q='__all__', extra_filter='(date:["2020-01-01" TO "2022-12-31"])', size='0', hist='date', hits=False)
        assert len(res['facets']['hist_dates']['terms']) > 1

    # test extra_filter
    def test_17(self):
        res = self.query(q='__all__', extra_filter='(healthCondition.name:("asthma")) AND -_exists_:measurementTechnique.name', 
                         size='0', facet_size='0', hits=False)
        assert res['total'] > 0
    ...
    def test_21(self):
        res = self.query(facets='conditionsOfAccess,includedInDataCatalog.name', q='_exists_:conditionsOfAccess')
        enum = ["Open", "Restricted", "Closed", "Embargoed"]
        for value in res['facets']['conditionsOfAccess']['terms']:
            assert value['term'] in enum, "%s is not a valid includedInDataCatalog entry. List of sources that have includedInDataCatalog: %s" \
                                            % (value['term'], res['facets']['includedInDataCatalog.name']['terms'])
    ...
    def test_201(self):
        ...
        for source_name, value in res['src'].items():
            assert 'sourceInfo' in value.keys(), "%s is missing the sourceInfo field." % source_name
            for field in source_fields:
                assert field in value['sourceInfo'].keys(), "%s is missing the %s field" % (source_name, field)
