...
class TestQueryGET(BiothingsWebTest):
    ...
    def test_15(self):
        res = self.query(
            q="__all__", extra_filter='(date:["2020-01-01" TO "2020-12-31"])', size="0", hist="date", hits=False
        )
    ...
    def test_16(self):
        res = self.query(
            q="__all__", extra_filter='(date:["2020-01-01" TO "2022-12-31"])', size="0", hist="date", hits=False
        )
        assert len(res["facets"]["hist_dates"]["terms"]) > 1
