...
def write_tsv(path, rows):
    fields = list(sync_module.RESOURCE_BASE_COLUMNS) + [
        "_ProdApproved?", "_ResCatProdApproved?",
    ]
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fields, delimiter="\t")
        writer.writeheader()
        writer.writerows({
            "_ProdApproved?": "TRUE", "_ResCatProdApproved?": "FALSE", **row,
        } for row in rows)
...
def test_prod_approval_preserves_unapproved_sources_and_catalog_links(
    ...
    originals = {
        "unapproved": {
            "_id": "unapproved", "name": "Live source", "identifier": "Live name",
            "url": "https://live.example.org", "schema": {},
        },
        "approved": {
            "_id": "approved", "name": "Old approved name",
            "identifier": "Indexed name", "url": "https://approved.example.org",
            "sameAs": "https://data.niaid.nih.gov/resources?id=dde_old",
            "schema": {},
        },
        "catalog_approved": {
            "_id": "catalog_approved", "name": "Old catalog name",
            "identifier": "Catalog indexed name",
            "url": "https://catalog.example.org", "schema": {},
        },
    }
    ...
    write_tsv(tsv, [
        {
            "name": "New live source", "url": "https://live.example.org",
            "_ProdApproved?": "FALSE", "_ResCatProdApproved?": "TRUE",
            "sameAs": "https://data.niaid.nih.gov/resources?id=dde_unapproved",
        },
        {
            "name": "New approved name", "url": "https://approved.example.org",
            "sameAs": "https://data.niaid.nih.gov/resources?id=dde_unapproved",
        },
        {
            "name": "New catalog name", "url": "https://catalog.example.org",
            "_ResCatProdApproved?": "TRUE",
            "sameAs": "https://data.niaid.nih.gov/resources?id=dde_approved",
        },
    ])

    built = sync_module.build(tsv)
    assert built["unapproved"] == originals["unapproved"]
    assert built["approved"]["name"] == "New approved name"
    assert built["approved"]["identifier"] == "Indexed name"
    assert "sameAs" not in built["approved"]
    assert built["catalog_approved"]["sameAs"].endswith("dde_approved")
