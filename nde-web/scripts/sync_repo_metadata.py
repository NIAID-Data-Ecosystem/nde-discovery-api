...
On production, only rows with ``_ProdApproved? = TRUE`` update source metadata.
Other rows keep their existing production JSON unchanged. The separate
``_ResCatProdApproved?`` flag controls whether ``sameAs`` may link the source
to a DDE ResourceCatalog record. ``identifier`` is preserved because the
portal uses it as the exact ``includedInDataCatalog.name`` search filter.
Source keys, schema mappings, schedules, and ingestion settings are preserved.
...
def resource_base_record(row: dict[str, str]) -> dict[str, Any]:
    """Extract approved descriptive fields without changing API shapes."""
    record: dict[str, Any] = {}
    for col, (field, coercer) in RESOURCE_BASE_COLUMNS.items():
        if field in RESOURCE_BASE_PRESERVED_FIELDS:
            continue
        if field == "sameAs" and row.get("_ResCatProdApproved?") != "TRUE":
            continue
...
def build(resource_base_tsv: Path | None = None) -> dict[str, dict[str, Any]]:
    ...
    # Keep existing production metadata for sources whose new curation is not
    # approved. Approved rows may update descriptive fields, but an unapproved
    # DDE ResourceCatalog must never be linked into sourceInfo via sameAs.
    for key, data in repos.items():
        data.setdefault("_id", key)
        tsv_row = find_resource_base_row(rows, key, data)
        if tsv_row is None:
            continue
        if tsv_row.get("_ProdApproved?") != "TRUE":
            continue
        priority_row = priority_by_key.get(key)
        if priority_row:
            merge_update(data, priority_row)
        if tsv_row.get("_ResCatProdApproved?") != "TRUE":
